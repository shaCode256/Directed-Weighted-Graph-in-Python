...
//                }
    ...
    ///////////////////////////////////////added due to EX3- compare with python//////////////////////////

    /**
     * Finds all the Strongly Connected Component(SCC) in the graph.
     *
     * @return: The list of all the SCC
     */

    ArrayList<NodeData> tropoligalSort = new ArrayList<>();
    ArrayList< ArrayList<NodeData>> sccList= new ArrayList<>();


    List connected_components() {
        dfs(myGraph);
        DWGraph_DS reversedGraph = reverse_graph(myGraph);
        dfsTropologic(reversedGraph);
        return sccList;
    }

    // dfs implementation
    void dfs(directed_weighted_graph myGraph) {
        for (api.node_data v : myGraph.getV()) {
            NodeData vertex = (NodeData) (v);
            vertex.color = "white";
            vertex.parent = null;
        }
        for (node_data ver : myGraph.getV()) {
            NodeData vertex = (NodeData) ver;
            if (vertex.color == "white") {
                DFSVISIT((DWGraph_DS) myGraph, vertex);
            }
        }
    }
    ...
    void DFSVISIT(DWGraph_DS G, NodeData vertex) {
        vertex.color = "gray";
        Set<Integer> edgesOutInt = G.getEdgesOutInt(vertex.getKey());
        for (int nodeNum : edgesOutInt) {
            NodeData v = (NodeData) G.getNode(nodeNum);
            if (v.color == "white") {
                v.parent = vertex;
                DFSVISIT(G, v);
            }
        ...
        vertex.color = "black";
        tropoligalSort.add(vertex);
    }

    //dfs implementation- second part//

    void dfsTropologic(DWGraph_DS G) {
        for (NodeData vertex : tropoligalSort) {
            vertex.color = "white";
            vertex.parent = null;
        }
    ...
    void DFSVISITTropoligic(DWGraph_DS G, NodeData vertex) {
        ArrayList<NodeData> scc = new ArrayList<>();
        vertex.color = "gray";
        Set<Integer> allOut= G.getEdgesOutInt(vertex.getKey());
        for (int vNum : allOut) {
            NodeData v = (NodeData) G.getNode(vNum);
            if (v.color == "white") {
                v.parent = vertex;
                DFSVISITTropoligic(G, v);
                scc.add(v);
            }
        }
        if (!scc.isEmpty()) {  //if the list of components is not empty
            sccList.add(scc);
        }
        vertex.color = "black";
     //   return sscList;
    ...
    private DWGraph_DS reverse_graph(directed_weighted_graph myGraph) {
        DWGraph_DS newGraph = new DWGraph_DS();
        for (node_data nd : myGraph.getV()) { //Reverse the original graph
            newGraph.addNode(nd); //add the nodes to the new graph
            for (edge_data edge : myGraph.getE(nd.getKey())) {
